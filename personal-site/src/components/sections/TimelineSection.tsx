      ...
      <div className="timeline-pin sticky top-0 h-screen overflow-hidden">
        ...
        <div className="section-transition-reveal pointer-events-none absolute left-6 top-24 z-10 max-w-[min(520px,84vw)] md:left-12">
          <p className="text-xs font-semibold uppercase tracking-[0.34em] text-white/35">
            <AnimatedText text="Experience Solar System" duration={760} staggerMs={8} />
          </p>
          <h2 className="iridescent-text mt-3 text-4xl font-bold sm:text-5xl">
            <AnimatedText text="Timeline" duration={780} />
          </h2>
          <p className="mt-5 max-w-xl text-sm leading-7 text-white/55">
            <AnimatedText
              text="从统计训练到 AI 产品、数据交付与商业化实验的阶段轨迹。"
              delay={0.04}
              duration={780}
              staggerMs={8}
            />
        ...
        <div className="section-transition-reveal pointer-events-none absolute right-6 top-[32vh] z-10 max-w-[min(460px,82vw)] text-right md:right-12 md:top-[24vh]">
          <div key={activeEntry.id} className="timeline-detail-swap">
            <div className="mb-5 flex flex-wrap justify-end gap-3 text-xs text-white/48">
              <span>
                <AnimatedText text={activeEntry.period} duration={720} staggerMs={6} />
              </span>
              <span>
                <AnimatedText text={typeLabel[activeEntry.type]} duration={720} staggerMs={6} />
              </span>
              <span>
                <AnimatedText text={activeEntry.organization} duration={720} staggerMs={6} />
              </span>
            </div>
            <h3 className="text-3xl font-bold leading-tight text-white md:text-4xl">
              <AnimatedText text={activeEntry.title} duration={780} />
            </h3>
            <p className="mt-5 text-sm leading-7 text-white/62">
              <AnimatedText text={activeEntry.description} delay={0.04} duration={780} staggerMs={7} />
            </p>
            <div className="mt-6 flex flex-wrap justify-end gap-3">
              {activeEntry.tags.map((tag, index) => (
                <span key={tag} className="cursor-target text-xs font-semibold text-white/46 transition-colors hover:text-teal-100">
                  <AnimatedText text={tag} delay={0.04 + index * 0.02} duration={720} staggerMs={6} />
                </span>
              ))}
            </div>
          </div>
