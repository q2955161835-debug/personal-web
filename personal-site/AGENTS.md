...
- `src/components/sections/`：页面分区，包括 Hero、About、Projects。
...
## 严格浏览器视觉验收规则
- 每次涉及 UI、3D、滚动、项目详情或响应式布局的改动，必须使用浏览器从首屏纵览到项目段，再返回上方页面做视觉验收。
- 验收至少覆盖桌面视口与一个移动视口；桌面需检查 Hero、About、Projects、项目详情打开/关闭、项目切换、从 Projects 回滚到上方页面。
...
- 当前仓库未配置 `git remote`，未确认 GitHub 地址与公开/私有状态；配置远程后需在本节补充。
