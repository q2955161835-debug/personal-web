...
- 当前交互目标：Projects 使用加长 DNA 键位浏览项目，项目说明固定在右侧并随滚动淡入淡出；Data Analysis 使用固定横向滚动的短彩色竖线坐标轴、`fluid-glass` 柱状图与方法星云；Experience 使用 R3F 太阳系穿梭时间线；全站鼠标视觉统一为淡彩虹折射环，粒子系统保留实时鼠标驱散与复原。
...
- Projects 段必须验证：Hero 粒子与 DNA 粒子分离、项目键固定在 DNA 对应位置，滚动切换节奏跟随滚轮速度，右侧项目说明与当前键位同步且不重复 DNA 标签内容，后景粒子不干扰 DNA。
...
- Data Analysis 必须验证：滚动到该区后页面 pin 固定，`fluid-glass` 柱状图随滚轮横向移动；柱体按含金量从左到右排序；柱体上不显示含金量数字；点击柱体或滚动进度切换详情；坐标轴由大量短彩色竖线组成并随当前柱体形成波峰，不使用旧粒子坐标轴；详情不显示时间和交付文件数量，`价值点` 文案必须改为 `核心`；方法星云默认不显示文字，只有当前项目使用的方法粒子变大并显示方法名。
