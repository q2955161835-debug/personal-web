...
export const coronaFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // 脉动
    float pulse = 0.75 + 0.1 * sin(uTime * 1.2);
    float alpha = fresnel * (0.18 + s * 0.15) * pulse;

    // 边缘色调压暗
    vec3 color = uColor * (0.4 + s * 0.3);
...
export const refractionVertexShader = /* glsl */ `
  ...
  void main() {
    ...
    vPosition = position;
    // 折射层轻微扭曲，模拟空间涟漪
    float ripple = snoise(position * 0.4 + vec3(uTime * 0.12)) * 0.08;
...
export const refractionFragmentShader = /* glsl */ `
  ...
  uniform float uTime;
  uniform vec3 uColorA;  // 暖色（红橙）
  uniform vec3 uColorB;  // 冷色（青）
  uniform vec3 uColorC;  // 紫色

  void main() {
    // 多层 Fresnel 模拟色散（不同波长折射率不同）
    float fresnelR = 1.0 - max(dot(vViewDirection, vNormal), 0.0);
    float fresnelG = pow(fresnelR, 1.4);
    float fresnelB = pow(fresnelR, 1.8);

    // 三色分离
    vec3 color = vec3(0.0);
    color.r = uColorA.r * fresnelR + uColorB.r * fresnelG + uColorC.r * fresnelB;
    color.g = uColorA.g * fresnelR + uColorB.g * fresnelG + uColorC.g * fresnelB;
    color.b = uColorA.b * fresnelR + uColorB.b * fresnelG + uColorC.b * fresnelB;
    ...
    // 仅边缘可见，alpha 低柔
    float alpha = pow(fresnelR, 2.5) * 0.18 * pulse;
    gl_FragColor = vec4(color, alpha);
  }
`;
